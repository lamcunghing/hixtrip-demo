...
import com.hixtrip.sample.app.service.callback.OrderPayCallbackFactory;
...
public class OrderController {
    ...
    @Autowired
    private OrderPayCallbackFactory orderPayCallbackFactory;
    ...
    public String order(@RequestBody CommandOderCreateDTO commandOderCreateDTO) {
        //登录信息可以在这里模拟
//        var userId = "";
        commandOderCreateDTO.setUserId("123");
        commandOderCreateDTO.setUserName("张三");
    ...
    public String payCallback(@RequestBody CommandPayDTO commandPayDTO) {
        //模拟用户信息
        commandPayDTO.setPayUserName("张三");
        //1、获取回调对象
        OrderPayCallback orderPayCallback = orderPayCallbackFactory.get(commandPayDTO);
        //2、调用回调逻辑
        return orderService.payCallback(orderPayCallback, commandPayDTO);
    }
